...
export default function RefundOperations({ data, onReconcile, onAction, onRepair }: Props) {
  ...
  return (
    <section className="rounded-[12px] border border-[#dbe3ed] bg-[linear-gradient(135deg,#ffffff_0%,#f6f9ff_100%)] p-6 shadow-sm">
      ...
      {selectedAction && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="dialog" aria-modal="true" aria-labelledby="refund-action-title"><div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.15em] text-[#023E8A]">Operator confirmation</p><h3 id="refund-action-title" className="mt-1 text-lg font-semibold capitalize text-[#18202b]">{selectedAction === "repair_amount" ? "Repair refund amount" : `${selectedAction} refund`}</h3></div><button type="button" onClick={() => setSelectedAction(null)} aria-label="Close"><X size={18} /></button></div><p className="mt-4 text-sm leading-6 text-[#4e5d6d]">{selectedAction === "repair_amount" ? `Correct the stored amount to ${money(current?.expected_amount, current?.currency ?? undefined)} using the server policy calculation. This does not contact the partner or move money.` : selectedAction === "settle" ? "Settlement immediately executes the real payment refund with the partner. Confirm that the policy amount and booking are correct before continuing." : "This action changes the partner refund lifecycle and is recorded in the operations audit trail."}</p><label className="mt-5 block text-sm font-medium text-[#18202b]">Reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} rows={3} className="mt-2 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-[#023E8A]" placeholder="Explain why this action is being taken" /></label><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setSelectedAction(null)} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Cancel</button><button type="button" onClick={submitAction} disabled={working || !reason.trim()} className={`rounded-lg px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50 ${selectedAction === "settle" ? "bg-red-700 hover:bg-red-800" : "bg-[#023E8A] hover:bg-[#012f69]"}`}>{working ? "Working…" : selectedAction === "settle" ? "Confirm settlement" : selectedAction === "repair_amount" ? "Confirm repair" : `Confirm ${selectedAction}`}</button></div></div></div>}
    </section>
  );
}
